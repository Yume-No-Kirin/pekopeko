"""
Provider-construction helper: maps a loaded config to a concrete extraction
Provider. Used by callers of extract_source(), never by pipeline.py itself.
"""
...
def build_configured_provider(cfg: PekopekoConfig) -> Provider:
    ...
    raise ConfigError(f"extraction has no provider implementation for llm_provider.active={active!r}")
