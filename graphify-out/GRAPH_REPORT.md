...
## Corpus Check
- 261 files · ~211,152 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2835 nodes · 5006 edges · 182 communities (148 shown, 34 thin omitted)
- Extraction: 93% EXTRACTED · 7% INFERRED · 0% AMBIGUOUS · INFERRED: 340 edges (avg confidence: 0.87)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `e0f9a082`
...
2. `make_proposal_file()` - 64 edges
3. `load_config()` - 62 edges
4. `extract_source()` - 59 edges
5. `ExtractionResult` - 57 edges
6. `OllamaProvider` - 56 edges
7. `OllamaProvider` - 55 edges
8. `ExtractionResult` - 46 edges
9. `OllamaProviderConfig` - 44 edges
10. `parse_frontmatter()` - 42 edges
...
Cohesion: 0.07
Nodes (52): Ingestion module for Pekopeko - data ingestion pipeline., ingest_source(), IngestionResult, process_source(), Path, Provider, Main ingestion pipeline for processing source files., Represents the result of an ingestion operation. (+44 more)
...
Cohesion: 0.05
Nodes (81): OllamaProvider, OllamaProviderConfig, ExtractionResult, Provider, Parse the JSON extraction result from the LLM response., Configuration for Ollama provider., Concrete implementation of Provider using Ollama API., Extract entities, events, and relationships from text using Ollama. Args: text:… (+73 more)

### Community 37 - "ExtractionResult"
Cohesion: 0.11
Nodes (21): ExtractionResult, Base interfaces for LLM providers used in ingestion., Result of an extraction operation., Run final verification of all acceptance criteria., run_final_verification(), Test that domain validation is consistent with specification., test_domain_validation_compliance(), AC8: ingest_source() without an explicit state_dir resolves the task-state… (+13 more)
...
Cohesion: 0.10
Nodes (44): ExtractionError, InvalidDomainError, Exception, Typed exceptions for the entity/event/relationship extraction pipeline., Raised when domain is not one of the allowed domains., Raised when frontmatter is missing/invalid, before any file is written., Base class for all extraction/ module errors., ValidationError (+36 more)

### Community 40 - "routes_ingestion.py"
Cohesion: 0.05
Nodes (65): Fixed domain enum, re-declared for the API layer's own early request-boundary…, get_config(), route, get_ingestion(), list_ingestions(), route, Ingestion endpoints (async, ADI-010 SS2): POST starts a background…, start_ingestion() (+57 more)
...
### Community 43 - "ExtractedAssertion"
Cohesion: 0.10
Nodes (24): ExtractedAssertion, Represents a single extracted assertion from source content., FixedIngestionProvider, Matches app.ingestion.providers.base.Provider. Always returns the same fixed…, TC-UC003-01: a modified source is treated as a brand new, independent source…, test_modified_source_produces_independent_new_source_no_linkage(), _ingest_two_assertions(), UC-011 (Review Queue, V1 individual-review slice - the best-covered UC in the… (+16 more)
...
Cohesion: 0.13
Nodes (40): accept_proposal(), reject_proposal(), make_proposal_file(), Unit tests for pipeline.accept_proposal (acceptance criteria 1, 3, 4, 5)., test_accept_already_accepted_proposal_raises_and_leaves_files_unchanged(), test_accept_already_edited_then_accepted_proposal_raises_on_second_accept(), test_accept_nonexistent_proposal_raises_proposal_not_found(), test_accept_proposal_after_edit_writes_assertion_from_edited_body_and_fields() (+32 more)

### Community 50 - "review/pipeline.py"
Cohesion: 0.11
Nodes (25): Flask, Flask app factory for the Pekopeko backend API (ADI-010): registers all route…, Config endpoint (sync, read-only, ADI-010 SS4): projects the already-loaded…, DomainMismatchError, InvalidProposalStatusError, Exception, Typed exceptions for the proposal review workflow., Raised when the caller-supplied domain does not match the proposal's own domain… (+17 more)

### Community 51 - "extract_source"
Cohesion: 0.10
Nodes (38): extract_source(), Path, Provider, FakeProvider, FlakyProvider, _full_extraction_result(), _last_task_state(), ExtractionResult (+30 more)
...
Cohesion: 0.06
Nodes (29): SourceReaderRegistry, _build_reader_registry(), ExtractionPipelineResult, Entity/Event/Relationship extraction pipeline orchestration. Implements SOURCE…, Named distinctly from providers.base.ExtractionResult (the raw…, Path, Protocol, Base interfaces for source readers used in extraction. Independent of… (+21 more)
...
Cohesion: 0.06
Nodes (62): ProposalNotFoundError, Raised when proposal_id does not resolve to a file under <domain>/proposals/., Raised when provenance.source_id does not resolve to a file under…, SourceNotFoundError, archive_proposal_version(), assertion_path(), _generate_assertion_id(), list_proposal_ids() (+54 more)
...
Cohesion: 0.14
Nodes (23): ApiError, Exception, Typed exceptions for the api/ orchestration layer itself (as opposed to errors…, Raised for invalid pagination query parameters (limit/offset)., Base class for exceptions raised by the api/ orchestration layer itself., ValidationError, Raised when frontmatter is missing/invalid, on read or write., ValidationError (+15 more)
...
Cohesion: 0.06
Nodes (55): Full path strings already proposed by not-yet-accepted Proposals…, Full existing folder paths already used under <domain>/assertions/ (canonical,…, scan_existing_assertion_folders(), scan_proposed_path_segments(), Path, Unit tests for the ingestion pipeline., Test that pipeline handles provider failures gracefully., Test source reader registry functionality. (+47 more)

### Community 114 - "edit_proposal"
Cohesion: 0.11
Nodes (36): edit_proposal(), proposal_history_dir(), make_entity_proposal_file(), make_event_proposal_file(), make_relationship_proposal_file(), make_source_file(), fixture, Path (+28 more)
...
Cohesion: 0.12
Nodes (15): ExtractionResult, FakeExtractionProvider, FakeIngestionProvider, Path, Shared test-only helpers for api/ tests. Kept out of conftest.py so test…, Poll the on-disk TaskState until it leaves pending/running, or timeout. The…, wait_for_terminal_status(), Extraction route tests (AC5): identical shape to test_ingestion_routes.py, kept… (+7 more)
...
### Community 127 - "get_proposal"
Cohesion: 0.11
Nodes (26): get_proposal(), list_proposals(), ProposalDetail, ProposalSummary, _validate_domain(), UC-009 (Cross-Domain Analysis, isolation slice) and UC-018 (Fictional Universe…, TC-UC009-02a: a proposal only exists under its own domain's folder - looking it…, TC-UC009-02b: a corrupted/hand-edited proposal whose folder domain and… (+18 more)
...
Cohesion: 0.10
Nodes (28): ExtractedEntity, ExtractedEvent, Provider, Protocol, Base interfaces for extraction LLM providers (ADI-008). Pipeline code depends…, local_id is a transient identifier scoped to a single extraction call, assigned…, Concrete implementation of the extraction Provider using Ollama. This is the…, FixedExtractionProvider (+20 more)
...
Cohesion: 0.30
Nodes (14): accept(), _check_domain(), edit(), get_organization_folders(), get_proposal_detail(), get_proposals(), route, Review endpoints (sync, ADI-010 SS3): thin pass-through to… (+6 more)
...
### Community 153 - "api/conftest.py"
Cohesion: 0.09
Nodes (36): create_app(), main(), HTTP/REST API layer for the Knowledge Core (TASK-007, ADI-010): exposes…, ApiSettings, load_settings(), MissingSettingError, Exception, API process startup settings (ADI-010 SS4/SS5): vault_root and the shared API… (+28 more)
...
### Community 172 - "_events_from_last_task_state"
Cohesion: 0.18
Nodes (11): _events_from_last_task_state(), TASK-001b AC1: a successful ingest_source call produces a TaskState.events…, TASK-001b AC3: a simulated provider failure appends a warning-level event…, Covers the 'Failed to write proposal' instrumentation branch: a warning event…, Covers the outer except-block instrumentation branch: an error raised outside…, TASK-001c AC5: an empty/whitespace-only source file fails the task with a…, test_empty_source_file_fails_before_provider_call(), test_proposal_write_failure_appends_warning_event() (+3 more)

### Community 173 - "test_duplicate_and_modified_ingestion.py"
Cohesion: 0.36
Nodes (7): _proposal_ids_on_disk(), UC-016 (Duplicate/Repeated Ingestion) and UC-003 (Novel Change & Staleness,…, TC-UC016-01 (ingestion): re-ingesting identical content is a no-op beyond the…, TC-UC016-02 (extraction): same guarantee as above, independent pipeline., _source_ids_on_disk(), test_duplicate_extraction_creates_no_new_files_and_skips_provider_call(), test_duplicate_ingestion_creates_no_new_files_and_skips_provider_call()
...
## Knowledge Gaps
- **999 isolated node(s):** `name`, `private`, `version`, `type`, `dev` (+994 more)
