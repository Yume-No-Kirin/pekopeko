...
Read `docs/ROADMAP.md` in full before touching anything in this repo. It is the single source of continuity — not this file. Then follow ROADMAP.md's own "Démarrage de session" steps: find the current step, read the matching "Lecture requise" file list in full (not a summary), and check `specs/decisions/` for the ADRs the current ticket cites.
...
- Everything else (this file included) should point at those two, not duplicate their content. A doc that restates them drifts out of date silently — that is exactly how an earlier context dump in `docs/` went stale and had to be dropped.
...
- Dependencies are pinned in `src/requirements.txt` (`pyyaml`, `requests`, `pytest`). No `pyproject.toml` exists yet.
...
- **Always check** if the content does not contain special or unreadable characters (Ã©,Â«,... for example).
- **JSON/YAML must never carry a BOM**: neither format allows one at the start of the file — a leading BOM makes some parsers fail outright and others fail silently on the first key. Same "never write a BOM" rule as above, called out explicitly here because the failure mode is worse (broken data, not just a display glitch).
- **Console/terminal output is not the file**: Windows cmd/PowerShell often isn't UTF-8 by default, so accented characters can look mangled in terminal output even when the underlying file is correctly encoded. Before concluding a file is corrupted, check `chcp 65001` / `PYTHONUTF8=1` rather than trusting what the terminal rendered.
- **If mojibake is already present** (sequences like `Ã©`, `Â«`, `â€™`), that's UTF-8 text that was decoded as cp1252/latin-1 and re-encoded — don't hand-patch individual characters. Regenerate the file from a known-good source, or fix it by decoding the raw bytes explicitly as UTF-8 rather than editing the garbled text visually.
- **Multi-agent repo**: another agent (qwen3-coder, via Codex CLI + an externalized Ollama on another machine) also writes files in this repo. Don't assume it followed these same rules — scan files it touched for a leading BOM or mojibake patterns before treating its output as clean, the same way you'd check your own.